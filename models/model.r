...
foreach(network_name = names(extra_networks)[140:min(length(extra_networks), 159)]) %do% {

 network <- extra_networks[[network_name]]
...
combined_df18<- bind_rows(infected_df, informed_df)

str(combined_df18)

write.csv(combined_df18, "C:\\Users\\s2607536\\Documents\\GitHub\\Competing_contagions\\26thdec_5.csv")

##ad any network features that were missed
calculate_modularity <- function(network) {
 # Community detection
 community <- cluster_louvain(network, weights = E(network)$weight)
 # Calculate metrics
 modularity_q <- modularity(community)  # Modularity
 community_sizes <- sizes(community)   # Sizes of communities
 ...
 num_communities <- length(community_sizes)  # Number of communities

 return(list(
   modularity = modularity_q, 
   avg_module_size_raw = avg_module_size_raw,
   module_size_variation = module_size_variation,
   num_communities = num_communities  # Add number of communities

 ))

}
network_stats <- data.frame()
# Loop over each unique network in the data
for (network_name in unique(data$network)) {
 ...
 if (network_name %in% names(largest_components)) {
   ...
   stats$network <- network_name  # Add network name to the results
...
# Join the calculated stats with the original dataset
